...
def pro_generator(x, res_building, res_training, latent_size):
    ...
    def BAN(x):
        ...
        return x
    def conv_layer(x, number, fmaps):
        with tf.compat.v1.variable_scope('Conv2D_{}'.format(number)):
            x = layers.conv2d(x, fmaps=fmaps, kernel=3)
            x = BAN(x)
        return x
    def dense_layer(x, number):
        ...
        return x
    def to_rgb(x, res):
        with tf.compat.v1.variable_scope('Output_{}'.format(res), reuse=tf.compat.v1.AUTO_REUSE):
            x = layers.conv2d(x, fmaps=1, kernel=1)
            x = layers.bias(x)
        return x
    def block(x, res):
        with tf.compat.v1.variable_scope('Block_{}'.format(res)):
            if res==2:
                ...
                x = conv_layer(x, number='{}_1'.format(res), fmaps=nbof_fmaps(res))
            return x
    decreasing_stage = 5
    res_latent = int(np.log2(latent_size))
    def nbof_fmaps(res): return min(latent_size, 2**(res_latent+decreasing_stage-res))
...
def pro_discriminator(inputs, res_building, res_training, latent_size):
    def BA(x):
        ...
        return x
    def from_rgb(x, res):
        with tf.compat.v1.variable_scope('Input_{}'.format(res), reuse=tf.compat.v1.AUTO_REUSE):
            x = layers.conv2d(x, fmaps=nbof_fmaps(res), kernel=1)
            x = BA(x)
        return x
    def conv_layer(x, number, fmaps):
        with tf.compat.v1.variable_scope('Conv2D_{}'.format(number)):
...
def pro_generator_v2(x, res_building, res_training, latent_size):
    # res: resolution, 2: 4x4, 3: 8x8, 4: 16x16, ...
    def BAN(x):
        x = layers.bias(x)
        x = layers.leaky_relu(x)
...
def pro_discriminator_v2(inputs, res_building, res_training, latent_size):
    def BA(x):
        x = layers.bias(x)
        x = layers.leaky_relu(x)
        return x
...
def pro_gan_v2(image_inputs, noise_inputs, latent_size, res_building, res_training, minibatch_size):
    # Define networks
    generator = network.Network('generator', pro_generator_v2, noise_inputs, res_building=res_building, res_training=res_training, latent_size=latent_size)
    discriminator = network.Network('discriminator', pro_discriminator_v2, image_inputs, res_building=res_building, res_training=res_training, latent_size=latent_size)
    # Retrieve network outputs
    fake_images = generator(noise_inputs)
    fake_outputs = discriminator(fake_images)
    real_outputs = discriminator(image_inputs)
